...
VersionInfo()
{
	...
	local Description="Scan(Compare) LAN and BMC MAC address, and save in file MAC#.TXT, BMCMAC#.TXT"
	...
	printf "%2s%-12s%2s%-s\n" "" "History" ": " ""
	# 日期,修改内容
	printf "%16s%-s\n" "" "2018-09-17,Update for Batch test mode"
	printf "%16s%-s\n" "" "2019-01-20,Add the tool: scanner for forbidden keyboard input"
	printf "%16s%-s\n" "" "2019-06-27,Add the control of range"
	echo
	exit 1
}
...
GetParametersFrXML  ()
{
	...
	ScanPriority=$(xmlstarlet sel -t -v "//Scan/TestCase[ProgramName=\"${BaseName}\"]/ScanPriority" -n "${XmlConfigFile}" 2>/dev/null)
	...
	do
		...
		do
			...
			if [ $? == 0 ] ; then
				...
				mac=$(cat ${WorkPath}/scan_${BaseName}.txt | sed 's/!/1/g' |sed 's/@/2/g' |sed 's/#/3/g' |sed 's/\$/4/g' |sed 's/%/5/g' |sed 's/\^/6/g' |sed 's/\&/7/g' |sed 's/\*/8/g' |sed 's/(/9/g' |sed 's/)/0/g')
				rm -rf ${WorkPath}/scan_${BaseName}.txt >/dev/null 2>&1
			else
				read mac
			fi	
			...
			if [ "${#CurMac[$i]}" == "0" ]; then
				...
				if [ $? == 0 ] ; then
					...
					if [ ${MacType} == 'BMC' ] && [ $Product != 0 ] && [ $ScanPriority == 'LAN' ] ; then
					...
					if [ ${MacType} == 'LAN' ] && [ $Product != 0 ] && [ $ScanPriority == 'BMC' ] ; then
						#for((L=1;L<=${LanAmount};L++))
						for((L=${BmcStartNumber};L<${BmcAmount}+${BmcStartNumber};L++))
						do
							TempMacAddr=$(cat ${SavePath}/BMCMAC${L}.TXT  2>/dev/null )
							DiffMac=$(echo ${TempMacAddr} | grep -i "${CurMac[$i]}" )
							if [ ${#DiffMac} != 0 ]	; then
								Process 1 "Found BMC and LAN MAC address files are same. Check MAC"
								echo "${CurMac[$i]} is the same as: ${SavePath}/BMCMAC${L}.TXT(${TempMacAddr}) "
								echo
								let Chance--
								continue 2
							fi
							...
							case ${BMC2LAN} in
								[Gg][Tt])
									if [ $Gap -ge 0 ]; then
										Process 1 "LANMAC(${CurMac[$i]}) is great than BMC MAC(${TempMacAddr})"
										echo
										let Chance--
										continue 2
									fi
								...
								[Ll][Tt])
									if [ $Gap -le 0 ]; then
										Process 1 "LANMAC(${CurMac[$i]}) is less than BMC MAC(${TempMacAddr})"
										echo
										let let Chance--
										continue 2
									fi
