...
# Version  : 1.0.0
# Description :
# S368E AST1060对S368D BIOS&BMC 进行unprovision
# Change list:
...
def config_format():
    """
    <TestCase>
        <ProgramName>make_unprovision</ProgramName>
        <config name="ErrorCode" value="NXF21|Make unprovision FAIL"/>
        ...
        <config name="wait_time" value="60"/>
...
def main():
    ...
    if _status == 'prov' or _status == "prov_finished":
        print(f"Will make un_provision by cmd: {config['unprovision_cmd']}")
        ...
        if _cur_status == 'prov_finished':
            ...
            echopass("Rot un_provision finished!")
        else:
            fail_action('Make unprovision failed, please contact PE for support!')
    elif _status == 'un_prov':
        promptg(f"Unprovision is already finished")
    else:
