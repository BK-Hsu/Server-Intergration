...
VersionInfo()
{
	...
	local Description="Main program for Linux"
	...
	printf "%2s%-12s%2s%-s\n" "" "History" ": " ""
	# 日期,修改内容
	printf "%16s%-s\n" "" "1.1.0更新 ：項目的index不需連續也可以進行測試,方便PE debug"
	printf "%16s%-s\n" "" "1.1.1更新 ：合并部分脚本到主程式"
	printf "%16s%-s\n" "" "1.1.2更新 ：更新程式清單是否有對應的配置檔內容/XML格式更新for組合測試"
	printf "%16s%-s\n" "" "1.1.3更新 ：ATA/BUS/Hardware Error等報錯修改為週期檢查"
	printf "%16s%-s\n" "" "1.1.4更新 ：适用于Ubuntu OS/更新對二進制可執行文件的管控(通過MD5)"
	printf "%16s%-s\n" "" "            更新測試結束後的退出方式: 關機或退出程式"
	printf "%16s%-s\n" "" "2020-12-31: 自定義FAIL最大次數,超過此次數刪除測試記錄"
	printf "%16s%-s\n" "" "1.2.0 更新: lan_c,bmcmac_c,cmostime 测试一次fail就会上传并删除资料"
	printf "%16s%-s\n" "" "			   兼容python 脚本，可以直接调用python脚本"
	printf "%16s%-s\n" "" "            支持并行测试程式"
	printf "%16s%-s\n" "" "1.2.1 更新: 修正开始测试前check log逻辑，确保每个item 测试都PASS过"
	printf "%16s%-s\n" "" "			   遇到组合程式跳过前面项目，先写入Proc ID,然后再加一"
	echo
	exit 1
}
...
DumpXML()
{
	cat <<-MainXM | xmlstarlet fo | tee ${BaseName}.xml
	<MSITEST version="v${ApVersion}" update="`date "+%Y-%m-%d"`">
		<Programs>
			<Item index="#" model="">/TestAP/Scan/ScanOPID.sh</Item>
			<Item index="#" model="">/TestAP/Scan/ScanFixID.sh</Item>
			<Item index="#" model="">/TestAP/Scan/ScanREV.sh</Item>
		</Programs>
		<Argument>
			<MainProg>
				<!-- For TestAP.sh -->
				...
				<ModelInfo>
					<!--機種測試信息-->
					<ModelName>609-S1651-010</ModelName>
					<BiosFile>ES165IME.120</BiosFile>
					<BmcFile>S165K131.ima</BmcFile>
					<EepromFile>I210I201.bin,I210I101.bin</EepromFile>
					<MpsFW>Config.csv</MpsFW>
				</ModelInfo>
		
				<ReleaseInfo>
					<!--PTE和PE信息,程式版本信息-->
					<PTEName>CodyQin</PTEName>
					<ReleaseDate>2020/05/03</ReleaseDate>
					
					<PEName>Jim Green</PEName>
					<Update>2020/05/03</Update>
				...
				<Pretest>
					...
					<FailLockAndUpload>
						...
						<FailLocking>Enable</FailLocking>
						
						<!-- Fail上傳Enable/Disable，超過4次fail上傳MES -->
						<FailUpload>Enable</FailUpload>
						<UrlAddress>
				...
				<UpLoad>
					...
					<Model>
						<ForModel>609-S165B-02S</ForModel>
						<KeyWord>REV TPVER MODEL</KeyWord>
						<FileList>S.TXT TPVER.TXT MODEL.TXT</FileList>
					</Model>
				</UpLoad>
			</MainProg>
			...
			<Scan>
				<TestCase>
					...
					<AutoScanProcVal>lan_c.sh</AutoScanProcVal>
...
IgnoreSpecTestItem()
{
	...
	cat -v "${MainDir}/PPID/SN_MODEL_TABLE.TXT" 2>/dev/null | grep -iwq "${SoleSuitForModels}"
	if [ $? == 0 ] ; then
...
Run() 
{
	...
	while :
	do
		# 在测试之前检查项目测试fail的次数，从最后一次该项目PASS之后的内容中进行查找，fix 相同项目，在前面的fail次数会记录到下一次测试当中，导致测试删除资料
		lastPassrecord=$(cat -v "${MainLog}" 2>/dev/null | grep -w "${ProgramName}" | tr -d ' '| grep -i "TestPassin" | tail -n1)
		lastPassrecord=${lastPassrecord:-"Nopassrecord"}
		if [ "${lastPassrecord}"x != "Nopassrecord"x ]; then
			CurFailCnt=$(cat -v "${MainLog}" 2>/dev/null | grep -w "${ProgramName}" | tr -d ' ' | grep -iwFA4000 "${lastPassrecord}" | grep -ic "TestFail")
		else
		...
		# For Batch Test
		IgnoreSpecTestItem ${ProgramName} ${TotalItemIndex[$z]} >/dev/null 2>&1
		if [ $? == 0 ] ; then
			if [ "$(${FullProgramName} -P ${XmlConfigFile} 2>&1 | grep -ic "ParallelTest")" == 0 ] ; then
				...
				{ 	echo "${FullProgramName##*.}" | grep -iwq "py"
					if [ $? == 0 ] ; then
						...
						${FullProgramName} -x ${XmlConfigFile}
					fi
					if [ $? == 0 ] ; then
						echo "0" >  ${MainDir}/PPID/result.tmp
					else
...
#				fi
			if [ "${CurFailCnt}" -le "${FailCntLimit}" ]; then
				...
				if [[ "${CurFailCnt}" -gt "${FailCntLimit}" && $(echo ${APVersion} | grep -ic "u") -ge 1 ]] ; then
					...
					# FailLockOrUpload "UPLOAD" | tee -a "${MainLog}"
					sh /TestAP/PPID/copyserver.sh "Fail"
				fi
...
ShowHeader ()
{
	...
	***************************************************************************
	...
	printf "%14s%-25s%-8s%-28s\n"  "Main Version:" " ${APVersion}" " md5sum:" " `md5sum ${MainDir}/${ShellFile} 2>/dev/null | cut -c 23-32` "
	...
	printf "%14s%-25s%-8s%-28s\n" "BIOS info:" "`if [ "${BiosFile}"x == "x" ] ; then echo -n " " && dmidecode -t0 | awk -F': ' '/Version:/{print $2}' | head -n1 | tr -d '\n'; else printf "%s\n" " ${BiosFile}" ; fi`" "Release:" " `dmidecode -t0 | grep "Release" | head -n1 | awk '{print $NF}'`"
	[ ${#BmcFile} != 0 ] && printf "%14s%-25s%-8s%-28s\n" "Firmware:" " ${BmcFile}"
	[ ${#EepromFile} != 0 ] && printf "%14s%-61s\n" "EERROM:" " ${EepromFile}"
	[ ${#MpsFW} != 0 ] && printf "%14s%-61s\n" "MPS Firmware:" " ${MpsFW}"
	printf "%14s%-61s\n" "BootHDD S/N:" " ${BootSN}"
...
CheckAssociatedFiles ()
{
	...
	do
		if [ -s "${MainDir}/${AssociatedFiles[$f]}" ] || [ -s "${MainDir}/${AssociatedFiles[$f]%%.sh}" ] ; then
			:
		else
...
Wait4nSeconds ()
{
	...
	do   
		...
		# Q|q) echo && return 1 >> Q|q) echo && exit 1 ;;
		case ${Ans:-h} in
		Y|y) echo && break;;
		Q|q) 
			echo 
			echo "exit" > ${MainDir}/PPID/.BreakOut.txt 2>/dev/null
			return 1
			;;
		 *) : ;;
		esac
...
CalculateMD5()
{
	...
	do
		...
		if [ $? == 0  ]; then
			...
			echo
			# type-1 
			#AllSricpts=($(find ${MainDir}/ -type f -maxdepth 2 2>/dev/null | grep -iwE "(sh|xml|py)" | grep -iEv 'lan[0-9]{1,3}_' | grep -iv 'run' | grep -iv ' ' | tr '\n' ' '))
			
			# type-2
			AllSricpts=($(xmlstarlet sel -t -v "//Programs/Item" -n "${XmlConfigFile}" 2>/dev/null | grep -iwE "(sh|xml|py)" | awk -F'|' '{print $1}' |  tr '\n' ' '))
			AllSricpts=($(echo "${AllSricpts[@]} ${MainDir}/${ShellFile}"))
			local OtherPath=(ChkLogic GetPrdctInfo MT DelLog Scan Config)
			for ((p=0;p<${#OtherPath[@]};p++))
			do
				AllSricpts=(${AllSricpts[@]} `find ${MainDir}/${OtherPath[$p]} -type f -maxdepth 2 2>/dev/null | grep -iwE "(sh|xml|py)"`)
			done
			# end type-2
			...
			if [ $? == 0 ]; then
				AllBinaryFiles=($(find ${MainDir}/ -type f -maxdepth 2 2>/dev/null | xargs -I {} file {} 2>/dev/null | grep -iw "LSB executable" | awk -F':' '{print $1}' ))
			else
				AllBinaryFiles=()
			fi
			...
			if [ -s ${StdMD5} ] ; then
				local SricptBinaryFilesCnt=$(echo ${AllFiles[@]} | tr ' ' '\n' | grep  -iwEvc "xml+$")
				local XMLCnt=$(echo ${AllSricpts[@]} | tr ' ' '\n' | grep  -iwEc "xml+$")	
				Process 0 "Sricpt and Binary files: ${SricptBinaryFilesCnt}, XML files: ${XMLCnt}, have been encrypted"
				break
			fi
		else
			command -v whiptail >/dev/null 2>&1
...
CheckMD5()
{
	...
	if [ -s "${StdMD5}" ] ; then
		...
		local PassList=($(md5sum -c ${StdMD5} 2>/dev/null | grep -i "OK" | awk -F':' '{print $1}' | grep "${SoleShellList}\|${ShellFile}" ))
	...
	[ "${#FailList[@]}" == "0" ] && return 0
	ShowTitle "MD5 verify tool for Linux Program"
...
declare InternalVersion=V${ApVersion}
...
echo "${CheckLogic}" | grep -iwq "enable" 
if [ $? == 0 ] ; then
	if [ -f "${MainDir}/ChkLogic/ChkLogic" ] ; then
		${MainDir}/ChkLogic/ChkLogic -Px ${XmlConfigFile} 2>/dev/null || exit 1
	elif [ -f "${MainDir}/ChkLogic/ChkLogic.sh" ] ; then
		${MainDir}/ChkLogic/ChkLogic.sh -Px ${XmlConfigFile} 2>/dev/null || exit 1
	else
		printf "%s\n" "No tool for checking logic ..."
	fi
fi
#CheckParameter
...
clear
ShowHeader 
#${MainDir}/Scan/ScanPPID.sh -x ${XmlConfigFile}
# for batch function test
for((C=0;C<3;C++))
do
	if [ -f "${MainDir}/Scan/ScanSNs" ] ; then
		${MainDir}/Scan/ScanSNs -x ${XmlConfigFile} 
	else
		${MainDir}/Scan/ScanSNs.sh -x ${XmlConfigFile}
	fi
...
if [ -e "${MainLog}" ] && [ ${#pcb} != 0 ] ; then
	...
	echo "${APVersion},${Update},`md5sum ${MainDir}/${ShellFile} ${MainDir}/PPID/ChkMD5/StdMD5 2>/dev/null | awk '{print $1}' |cut -c 29-32 | tr -d '\n'`" >${MainDir}/PPID/TPVER.TXT
...
if [ ${EndIndex} == 0 ];then
	...
	if [ $index_flag != 3 ];then
		Process 1 "并行测试参数 StartParalle, StartIndex, EndIndex 不在对应的测试清单中，请检查设定！！"
		exit 3
	fi
fi
	...
	case ${#ProgramNameSet[@]} in
		...
		*)	
			...
			if [ $ProcID -gt ${StartIndex} ] && [ $ProcID -le ${EndIndex} ];then
				while :
				do					
					ps -ax | grep -i "Paralle.sh" |grep -iv "grep" |grep -iv "gedit" >/dev/null 2>&1
					if [ $? == 0 ];then
						echo "The background process is still running,please waiting" | tee -a ${MainLog}
						sleep 2
					else
						cat -v /TestAP/PPID/.ParalleFinishflag 2>/dev/null |grep -iwq "ok"
						if [ $? != 0 ];then
							printf "\e[1;31m%s\e[0m\n" "并行程式测试Fail退出或者异常停止，请确认，主程式等待中...."
							echo -ne "请确认是否重新打开并行测试？ [ \e[32mY/Enter\e[0m ]=重新打开并行测试. [ \e[31mN\e[0m ]=Exit."
							read -n 1 answer
							echo ''
							case ${answer:-y} in
							Y|y)
								echo "测试员重新运行并行程式运行在： `date "+%Y-%m-%d %H:%M:%S %A %Z"`" | tee -a "${MainLog}"
			...
			TestPassCnt=0
			for ((x=0;x<${#ProgramNameSet[@]};x++))
			do
				cat -v ${MainLog} 2>/dev/null | grep -w ${ProgramNameSet[$x]} | tr -d ' ' | grep -iq "TestPass"
				if [ $? == 0 ]; then
					let TestPassCnt++
				fi
			done
			...
			if [ ${TestPassCnt} -ge ${#ProgramNameSet[@]} ] && [ $ProcID  -gt ${StartIndex} ] && [ $ProcID -le ${EndIndex} ]; then
				...
				ps -ax |grep -i "Paralle.sh" |grep -iv "grep" |grep -iv "gedit" >/dev/null 2>&1
					if [ $? != 0 ] && [ ! -e /TestAP/PPID/.ParalleFinishflag ];then
					gnome-terminal --window -- bash -c "/TestAP/Test/Paralle.sh -x ${XmlConfigFile};exec bash"
					fi
				fi
				clear
				if [ ${#ProgramNameSet[@]} -gt 1 ] ; then
					...
					do	
						...
						if [ $? == 0 ] ; then
							...
							#echo "${ProgramFileSet[$T]}" | sed 's/|/ test pass, because no found any model name /g'  >> "${MainLog}"
							echo "${ProgramFileSet[$T]} test pass in: `date "+%Y-%m-%d %H:%M:%S %Z"`" 2>&1 >> "${MainLog}"
							echo "${ProgramFileSet[$T]} test pass, because no found any model name " >> "${MainLog}"
...
# Upload test log file and upload test result to the Server(10.102.0.110)
sh ${MainDir}/PPID/copyserver.sh
