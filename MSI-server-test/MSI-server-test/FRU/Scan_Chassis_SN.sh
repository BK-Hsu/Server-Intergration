...
{
	ApVersion="1.1.1"
	local CreatedDate="2019-03-11"
	local UpdatedDate="2019-06-27"
	local Description="Scan REV and save in file REV.TXT"
...
DumpXML()
{
	...
	<Scan>
		<TestCase>
			...
			<!-- ScanREV.sh: Length=0表示no limited -->
			<Length>8</Length>
			<prompt>chassis serial</prompt>
			<SavePath>/TestAP/Scan/chassis_sn.txt</SavePath>
		</TestCase>
	</Scan>
	Sample
	sync;sync;sync
...
GetParametersFrXML  ()
{
	...
	prompt=$(xmlstarlet sel -t -v "//Scan/TestCase[ProgramName=\"${BaseName}\"]/prompt" -n "${XmlConfigFile}" 2>/dev/null)
...
main ()
{
	...
	do
		BeepRemind 0
		PrintfTip "Please scan ${sLength}-bit ${prompt} string, eg.: 010" 2>/dev/null
		echo -ne "Scan \033[31m${sLength}-bit\033[0m ${prompt} string: _______________\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b" 
		which scanner >/dev/null 2>&1
		if [ $? == 0 ] ; then
			rm -rf ${WorkPath}/scan_${BaseName}.txt >/dev/null 2>&1
			scanner ${WorkPath}/scan_${BaseName}.txt || continue
			read Rev<${WorkPath}/scan_${BaseName}.txt
			rm -rf ${WorkPath}/scan_${BaseName}.txt >/dev/null 2>&1
		else
			read Rev
		fi			
		...
		if [ "${Length}"x == "0"x ] || [ ${#Length} == 0 ] ; then
			echo $Rev | grep -E "^[0-9_A-Za-z]{1,50}" 
		else
			echo $Rev | grep -E "^[0-9_A-Za-z]{${Length}}+$"
		fi
	
		if [ $? != 0 ]; then
			Process 1 "Invalid ${prompt}: $Rev"
			printf "%-10s%-60s\n" "" "Try again ... "
			echo
			continue	
		fi
		...
		echo "$Rev" > $SavePath
		sync;sync;sync
		if [ $(cat "${SavePath}" | grep -iEc "[0-9A-Z]") != 0 ]; then
			echoPass "${prompt}: $Rev, write to: $SavePath"
			break
		else
			echoFail "Invalid file: $SavePath"
			echo "Try again ... "
			echo
		fi
	done
