...
VersionInfo()
{
	...
	local Description="Compare LAN MAC address"
...
`basename $0` [-x lConfig.xml] [-DV]
	...
	return code:
		0 : Compare lan mac address pass
		1 : Compare lan mac address fail
		2 : File is not exist
		3 : Parameters error
	    Other : Fail
...
ShowMTProcess ()
{
	...
	ShowTitle "Multithreading tool for LAN MAC compared test"
...
MergeLog()
{
	...
	echo "Multithreading test tool for LAN MAC compared test, all test logs" >> ./logs/${BaseName}.log
	...
	do
		...
		if [ $(echo "${MacFirst6}" | grep -ic 'FFFFFF' ) == 0 ] ; then
			if [ $(cat -v ${MacAddrFiles[$q]} | cut -c 1-6 | grep -ic "${MacFirst6}") != 1 ] ; then		
				Process 1 "Check the first 6bit of ${MacAddrFiles[$q]} fail"
...
CreatedCompareMAC ()
{
	for ((m=0;m<${#NicIndex[@]};m++))
	do
		let M=$m+1
		
		rm -rf "${BaseName}${NicIndexArray[$m]}.sh" 2>/dev/null
	cat <<-CompareMACFile > ${BaseName}${NicIndexArray[$m]}.sh
		echoPass()
		 { 	local String=\$@ 
			echo -en "\e[1;32m \$String\e[0m"
			str_len=$(echo \${#String}) 
		...
		# Dump the mac
		# If the current mac is equal to the target mac address,check pass
		DumpMac[\$TargetNicIndex]=\$(${ProgramTool} /nic=\${TargetNicIndex} /mac_dump | grep 'LAN MAC Address' | awk '{print \$6}' | tr -d ' .')
		if [ "\${DumpMac[\$TargetNicIndex]}x" == "\${TargetMacAddr}"x ] ; then
			echoPass "Compare LAN\${TargetNicIndex} mac address(\${TargetMacAddr})"
			NewMacAddrFile=\$(echo ${MacAddrFiles[$m]} | awk -F'/' '{print \$NF}' | tr [a-z] [A-Z])
			cp ${MacAddrFiles[$m]} ../PPID/\${NewMacAddrFile} 2>/dev/null
			sync;sync;sync
			
			# Confirm the MAC.TXT is valid
			MacAddrTemp=\$(cat -v ../PPID/\${NewMacAddrFile} 2>/dev/null | head -n1)
			if [ -z "\$MacAddrTemp" ] ; then
				echoFail "../PPID/\${NewMacAddrFile} abnormal or empty"
				GenerateErrorCode
				exit 1
			fi
			exit 0
		else
			echoFail "Compare LAN\${TargetNicIndex} mac address"
			echo "Current MAC Address: \${DumpMac[\$TargetNicIndex]}"
			echo "Scan in MAC Address: \${TargetMacAddr}"
			GenerateErrorCode
			exit 1
		fi
		}	
	...
	CompareMACFile
		...
		echo "GetMacAddr ${NicIndexArray[$m]} ${MacAddrFiles[$m]} ${LanChipset[$m]}" >> ${BaseName}${NicIndexArray[$m]}.sh
		sync;sync;sync	
...
main ()
{
	...
	if [ "${TestMode}"x == "serial"x ] && [ ${#pcb} != 0 ] ; then
		if [ ${ErrorFlag} != 0 ] ; then
			echoFail "MAC address dump and comparet"
		else
			echoPass "MAC address dump and compare"
		fi >> ${logpath}/${pcb}.log
		sync;sync;sync 
	fi	
	if [ ${ErrorFlag} != 0 ] ; then
		echoFail "MAC address dump and compare"
		GenerateErrorCode
		exit 1
	else
		echoPass "MAC address dump and compare"
...
do
	 case ${argv} in	
		...
		P)
			...
			if [ $? == 0 ] ; then
				printf "%-s\n" "ParallelTest,CheckLanMAC"
			else
				printf "%-s\n" "SerialTest,CheckLanMAC"
			fi
			exit 1
		;;		
