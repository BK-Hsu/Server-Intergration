...
VersionInfo()
{
	...
	local Description="Fault or Error LED function test"
...
`basename $0` [-x lConfig.xml] [-DV]
	...
	return code:
		0 : Fault or Error test pass
		1 : Fault or Error test fail
		2 : File is not exist
		3 : Parameters error
	    Other : Failure
...
GetParametersFrXML  ()
{
	...
	Location=${Location:-'FaultErrorLED'}
	if [ ${#Location} == 0 ] ; then
...
FaultErrorLEDLightOnTest ()
{
	...
	# Check Fault&Err LED
	ShowMsg --b "Fault or error LED( $Location ) test program is runing ..."
	ShowMsg --2 "Observe the LED lighting on how many times ..."
	ShowMsg --e "Comfirm the color and input the color code to test ..."
	echo -e "\e[33m Lighting on Fault or error led, pay attention please ...\e[0m"

	CpuTemp=$(ipmitool sensor | awk '/degrees C/&&/ok/&&/(CPU|cpu)/{print $0}' | head -n 1)
	if [ ${#CpuTemp} == 0 ]; then
		Process 1 "Get the CPU tempurature address"
		exit 4
	fi

	sensor=$(echo "${CpuTemp}" | awk '{print $1}')
	unr=$(echo "${CpuTemp}" | awk '{print $16}')
	ucr=$(echo "${CpuTemp}" | awk '{print $18}')
	unc=$(echo "${CpuTemp}" | awk '{print $20}')
	echo "${sensor}, $unr $ncr $unc"

	RandomNumber=$(echo $((RANDOM%4+3)))
	SleepTime=$(echo "obase=10; ibase=10; (6-${RandomNumber})*1.9" | bc)

	#Turn off Fault&Error LED
	ipmitool sensor thresh "${sensor}" upper "${unc}" "${ucr}" "${unr}" > /dev/null 2>&1
	sleep 2

	for (( i=0;i<${RandomNumber};i++ ))
	do
	   #Turn on Fault&Error LED
	   ipmitool sensor thresh "${sensor}" upper 0 "${ucr}" "${unr}" > /dev/null 2>&1
	   sleep 1
	   #Turn off Fault&Error LED
	   ipmitool sensor thresh "${sensor}" upper "${unc}" "${ucr}" "${unr}" > /dev/null 2>&1
	   sleep 1.5
...
declare XmlConfigFile FaultErrorConfigFile ColorCode Location
