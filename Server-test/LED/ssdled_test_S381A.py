...
class MG9200:
    ...
    def bin2_str2(self) -> tuple[dict, dict]:
        ...
        return {'hex_str': _hex_str1, 'led_cnt': bin_data1.count("1")}, {'hex_str': _hex_str2,
                                                                         'led_cnt': bin_data2.count("1")}
...
def create_command() -> str:
    ...
    if _command_type == 'raw_type':
        if config['raw_bus_convert'] == "disable":
...
def led_blink_test(test_list: list[MG9200], blink_set: list[dict]):
    ...
    _blink_prompt_list = ["绿色输入1"]
    for _single_set in blink_set:
        _single_prompt = "".join([str(_single_set['prompt']), "输入", str(_single_set['ans_code'])])
        _blink_prompt_list.append(_single_prompt)
    _blink_prompt = ",".join(_blink_prompt_list)
    for item in blink_set:
        ...
        user_input = int(input(f"\033[1;33m请输入SSD LED闪烁的颜色, {_blink_prompt}: \033[0m"))
...
def main():
    ...
    color_set = [{"color": "Amber", "code": "44", "prompt": "琥珀色"}]
    blink_set = [{"color": "Amber", "code": "46", "prompt": "琥珀色", "ans_code": 2}]
    debug_set = [{"color": "Green", "code": "0x58 3", "prompt": "绿色"}]
