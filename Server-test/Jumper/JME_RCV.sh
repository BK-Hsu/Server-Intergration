...
VersionInfo()
{
	...
	local UpdatedDate="2018-06-29"
	local Description="ME recovery jumper status test"
...
`basename $0` [-x lConfig.xml] [-DV]
	...
	return code:
		0 : ME recovery jumper status test pass
		1 : ME recovery jumper status test fail
		2 : File is not exist
		3 : Parameters error
	    Other : Failure
...
declare XmlConfigFile JMEConfigFile Location Command NormalCode ShortCode TestMode
#Change the directory
cd ${WorkPath}/JME >/dev/null 2>&1 
