#!/bin/bash
#FileName : UncontrollableJumper.sh
...
GetParametersFrXML  ()
{
	...
	DisplayMode=$(xmlstarlet sel -t -v "//Jumper/TestCase[ProgramName=\"${BaseName}\"]/DisplayMode" -n ${XmlConfigFile} 2>/dev/null | tr '[A-Z]' '[a-z]')
	JumperTotalAmount=$(xmlstarlet sel -t -v "//Jumper/TestCase[ProgramName=\"${BaseName}\"]/Member/Location" -n ${XmlConfigFile} 2>/dev/null | grep -iEc "[0-9A-Z]")
	for((J=1;J<=${JumperTotalAmount};J++))
	do
		JumperOptionIndex=($(xmlstarlet sel -t -v "//Jumper/TestCase[ProgramName=\"${BaseName}\"]/Member[${J}]/Option/@index" -n ${XmlConfigFile} 2>/dev/null | sort -u | sort -ns ))
		if [ ${#JumperOptionIndex[@]} != ${JumperOptionIndex[-1]} ] ; then
			Process 1 "Invalid setting //Jumper/TestCase[ProgramName=\"${BaseName}\"]/Member[${J}]/Option/@index"
			let ErrorFlag++
		fi
	done
...
{
	local Location=$1
	local CorrectlyOptionIndex=$(xmlstarlet sel -t -v "//Jumper/TestCase[ProgramName=\"${BaseName}\"]/Member[Location=\"${Location}\"]/CorrectlyOptionIndex" ${XmlConfigFile} 2>/dev/null)
	local TotalOption=$(xmlstarlet sel -t -v "//Jumper/TestCase[ProgramName=\"${BaseName}\"]/Member[Location=\"${Location}\"]/Option" ${XmlConfigFile} 2>/dev/null | grep -iEc "[0-9A-Z]")
	local RandomOption=($(RandomNoRepetition ${TotalOption}))
	local String=""
	ChkExternalCommands "whiptail"
	echo
	echo "**********************************************************************"
	printf "%-s\e[1;33m%-s\e[0m%-s\n" " Which decription best fits \"" "${Location}" "\" ?"
	echo "----------------------------------------------------------------------"
	for((r=0;r<${#RandomOption[@]};r++))
	do
		OptionContent=$(xmlstarlet sel -t -v "//Jumper/TestCase[ProgramName=\"${BaseName}\"]/Member[Location=\"${Location}\"]/Option[@index=\"${RandomOption[r]}\"]" -n ${XmlConfigFile} 2>/dev/null )
		String=$(echo ${String} \"${OptionContent}\"  \" \"  \"OFF\")
		printf "%-5s\e[1m%-s\e[0m\n" " [$((r+1))]" "${OptionContent}" 
	...
	#OPTION=$(whiptail --title "Menu Dialog" --menu "Which decription best fits \"${Location}\" ?" 15 60 4 ${String} 3>&1 1>&2 2>&3)
	rm -rf msg 2>/dev/null
	cat <<-Msg >msg
	#!/bin/bash
	Chosen=\$(whiptail --title "Test Dialog" --radiolist "Which decription best fits \"${Location}\" ?" 15 60 4 ${String} 3>&1 1>&2 2>&3)
	exitstatus=\$?
...
RandomShowOption()
{
	...
	# ****************************************************************
	#  Which decription best fits "JPWR1"?
	# ----------------------------------------------------------------
	#  [1] No jumper on the JPWR1
	#  [2] There's a jumper hat on the JPWR1
	#  [3] There's two jumper hat on the JPWR1
	# ****************************************************************
...
main ()
{
	...
	do
		local JumperLocation=$(xmlstarlet sel -t -v "//Jumper/TestCase[ProgramName=\"${BaseName}\"]/Member[${RandomJumper[j]}]/Location" -n ${XmlConfigFile} 2>/dev/null )
		local OptionCount=$(xmlstarlet sel -t -v "//Jumper/TestCase[ProgramName=\"${BaseName}\"]/Member[${RandomJumper[j]}]/Option" -n ${XmlConfigFile} 2>/dev/null | grep -iEc "[0-9A-Z]" )
		...
		do
			if [ ${DisplayMode} == "box" ] ; then
				...
				printf "%-s\e[1;33m%-s\e[0m%-s" "Which decription best fits " "${JumperLocation}" "? Input < ${ShowOption}>: "
				read -t 15 -n1 Answer
				echo -e "\b${Answer}"
				Answer=${Answer:-99}
