...
main ()
{
	...
	if [ ${#XmlConfigFile} != 0 ] ; then
		...
		if [ ${LastStatus} != ${Enable} ] ; then
			...
			echo "The last status in the test mode should be: $Disable,$Enable"
			exit 4
		fi
	fi

	echo ${BaseName} | grep -iq "Disable"
	if [ $? == 0 ] ; then
...
declare XmlConfigFile JSCUConfigFile Location Command Enable Disable TestMode BusID
