...
VersionInfo()
{
	...
	local Description="set the first boot as TeDoLi"
...
GetParametersFrXML  ()
{
	...
	TargetTedoliLog=$(xmlstarlet sel -t -v "//Grub4EFI/TestCase[ProgramName=\"${BaseName}\"]/TedoliLog" -n "${XmlConfigFile}" 2>/dev/null)
	TargetDeviceLog=$(xmlstarlet sel -t -v "//Grub4EFI/TestCase[ProgramName=\"${BaseName}\"]/DeviceLog" -n "${XmlConfigFile}" 2>/dev/null)
...
main()
{
	...
	do
		...
		DeviceLog=$(ls ${TargetDeviceLog:-"NULL"} 2>/dev/null | grep -iw "log" | tail -n 1)
		TedoliLog=$(ls ${TargetTedoliLog:-"NULL"} 2>/dev/null | grep -iw "log" | tail -n 1)
		
		if [ -f "${TedoliLog}" ] || [ -f "${DeviceLog}" ] ; then
			rm -rf ${TedoliLog} ${DeviceLog} 2>/dev/null
			Process 0 "Remove ${TedoliLog} ${DeviceLog}"
		fi
		if [ -f "${GrubMenu}" ]; then
...
declare TargetTedoliLog TargetDeviceLog
