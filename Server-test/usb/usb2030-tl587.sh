...
CheckAmount ()
{
	...
	do
		...
		USB30BlockAddr=($(ls -l /sys/block/ | grep -wE "usb[0-9]{1,4}" | grep -w "$SoleUSB30Addr" | awk -F'->' '{print $2}' | tr -d ' '))
		USB20BlockAddr=($(ls -l /sys/block/ | grep -wE "usb[0-9]{1,4}" | grep -w "$SoleUSB20Addr" | awk -F'->' '{print $2}' | tr -d ' '))
...
INFO
	...
	do
		...
		#Check exist
		LastDetectMsg=$(dmesg | grep -iw "${USB20Addr[$u]}" | grep -i "new.*speed USB device number" | tail -n1 )
		LastDetectMsg=${LastDetectMsg:-"NoExistDevice"}
		
		dmesg | grep -iw "${USB20Addr[$u]}" | grep -iwFA20 "${LastDetectMsg}" | grep -iwq "USB disconnect"
		...
		#Get USB Spec.
		USB20Spec=$(dmesg | grep -iw "${USB20Addr[$u]}" | grep -i "new.*speed USB device number" | tail -n1 | tr ' ' '\n' | grep -i 'high\|speed' | tr '\n' ' ')
	...
	do
		...
		#Check exist
		LastDetectMsg=$(dmesg | grep -iw "${USB30Addr[$u]}" | grep -i "new.*speed .*USB device number\|new.*speedPlus .*USB device number" | tail -n1  )
		LastDetectMsg=${LastDetectMsg:-"NoExistDevice"}
		dmesg | grep -iw "${USB30Addr[$u]}" | grep -iwFA20 "${LastDetectMsg}" | grep -iwq "USB disconnect"
		if [ $? == 0 ] || [ "${LastDetectMsg}"x == "NoExistDevice"x ] ; then
			printf "\e[1;31m%-14s\e[0m%-13s%-14s%-5s\n"  "Uninstall" "----" "----" "----"
			let ErrorFlag++
			continue
		...
		#Get USB Spec.
		USB30Spec=$(dmesg | grep -iw "${USB30Addr[$u]}" | grep -i "new.*speed .*USB device number\|new.*speedPlus .*USB device number" | tail -n1 | tr ' ' '\n' | grep -i 'speed')
		...
		#Get Product
		USB30Product=$(dmesg | grep -iw "${USB30Addr[$u]}" | grep "Product" | tail -n1 | awk -F':' '{print $NF}')
		...
		#Get Manufacturer
		USB30Manufacturer=$(dmesg | grep -iw "${USB30Addr[$u]}" | grep "Manufacturer" | tail -n1 | awk -F':' '{print $NF}')
		if [ ${#USB20Manufacturer} != 0 ] ; then
			printf "%-14s" "Pass"
		else
			printf "\e[1;31m%-14s\e[0m" "Fail"
		...
		#Get S/N
		USB30SerialNumber=$(dmesg | grep -iw "${USB30Addr[$u]}" | grep "SerialNumber" | tail -n1 | awk -F':' '{print $NF}')
