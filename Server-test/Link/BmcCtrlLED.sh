...
main()
{
	...
	if [ "${CountingStatus}" == "off" ] ; then
		## all on
		ipmitool i2c bus=6 0x40 0x00 0x06 0x00 0x00 >/dev/null 2>&1
		ipmitool i2c bus=6 0x46 0x00 0x06 0x00 0x00 >/dev/null 2>&1
		...
		do
			...
			do
				...
				ipmitool i2c bus=6 0x40 0x00 0x06 0x${bit[0]} 0x${bit[1]} >/dev/null 2>&1
				ipmitool i2c bus=6 0x46 0x00 0x06 0x${bit[2]} 0x${bit[3]} >/dev/null 2>&1
				...
				if [ $? == 0 ] ; then
					Process 0 "第$((c+1))組測試, 熄滅的數量正確(${bit2Bin: -TotalAmount})..."
				else
					Process 1 "第$((c+1))組測試, 熄滅的數量錯誤(${bit2Bin: -TotalAmount})..."
					printf "%10s%s\n" "" "熄滅LED正確數量是: ${TurnOffCount} PCs"
					let Error++
					echo -e "\n重新測試..."
					continue 2
				fi
		...
		## all off
		ipmitool i2c bus=6 0x40 0x00 0x06 0xff 0xff >/dev/null 2>&1
		ipmitool i2c bus=6 0x46 0x00 0x06 0xff 0xff >/dev/null 2>&1
		...
		TurnOffAll=$(echo ${LedSwitchStr} | tr 'x' "${TurnOffCode}" | cut -c 1-${TotalAmount} )
		printf "%s\n" "obase=16;ibase=2;${TurnOffAll}" | bc | tr '[A-Z]' '[a-z]'
		...
		do
			...
			do
				...
				bit2Bin=$(echo ${bit[@]} | tr -d ' ' | tr '[a-z]' '[A-Z]')
				bit2Bin=$(printf "%s\n" "obase=2;ibase=16;${bit2Bin}" | bc )
				bit2Bin=$(echo ${TurnOffCode}${TurnOffCode}${TurnOffCode}${TurnOffCode}${TurnOffCode}${TurnOffCode}${TurnOffCode}${TurnOffCode}${TurnOffCode}${TurnOffCode}${bit2Bin})
				...
				if [ $? == 0 ] ; then
					Process 0 "第$((c+1))組測試, 點亮的數量正確(${bit2Bin: -TotalAmount})..."
				else
					Process 1 "第$((c+1))組測試, 點亮的數量錯誤(${bit2Bin: -TotalAmount})..."
					printf "%10s%s\n" "" "點亮LED正確數量是: ${TurnOnCount} PCs"
					let Error++
					echo -e "\n重新測試..."
					continue 2
				fi
