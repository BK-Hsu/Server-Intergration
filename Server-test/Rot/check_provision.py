...
def config_format():
    """
    <TestCase>
        <ProgramName>check_provision</ProgramName>
...
def check_provision(config):
    _provision_sta = run_shell(config['provision_cmd']).stdout.strip()
    if _provision_sta != config['provision_status']:
        fail_action(f"PFR provision status({_provision_sta}) is wrong, should be {config['provision_status']}")
    else:
