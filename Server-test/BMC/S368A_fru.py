...
def switch_mux(command: str, step_list: list[dict]) -> bool:
    ...
    """
    stop_hwm_cmd = "ipmitool raw 0x38 0x41 1 0"
    stop_res = run_shell(stop_hwm_cmd)
    if stop_res.returncode != 0:
        fail_action("stop hwm fail")
    for step in step_list:
        ...
        run(switch_command, shell=True)
        time.sleep(0.5)
        # read_mux_cmd = f"{command} 0x{step['mux_addr']} 1 0"
        # read_res = run_shell(read_mux_cmd)
        # if read_res.returncode != 0:
        #    return False
        # else:
        #     if read_res.stdout.strip()[:2] != step['mux_channel']:
        #         return False
    return True
...
def config_format():
    """
    <TestCase>
        <ProgramName>backplane_check</ProgramName>
        ...
        <config1 smbus_type="fru" addr="a0" command="0x05 0x00 0x52" location="U23" stand_value="53 33 36 38 4d"/>
    ...
    for item in item_list:
        ...
        if item['smbus_type'] == 'temp':
            ...
            if int(_temp_lower) <= _res <= int(_temp_up):
                echopass(f"Backplane {item['location']} Temp: {_res} degrees check")
            else:
                promptr(f"Backplane {item['location']} Current Temp: {_res}, not between {_temp_lower} and {_temp_up} degrees")
