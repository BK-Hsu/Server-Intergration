...
{
	ApVersion="1.0.1"
	local CreatedDate="2020-11-19"
	local UpdatedDate="2020-12-18"
	local Description=" NMI button test"
...
`basename $0` [-x lConfig.xml] [-DV]
	...
	return code:
		0 : NMI button test pass
		1 : NMI button test fail
		2 : File is not exist
		3 : Parameters error
	    Other : Failure
	...
	<BMC>
		<TestCase>
			...
			<Button>
				...
				<!--DeteteCmd>ipmitool raw 0x38 0x12 0x84</DeteteCmd-->
				<DeteteCmd>ipmitool raw 0x38 0x12 0xca</DeteteCmd>
				<!--按鍵閉合狀態返回的值-->
				<Short>00</Short>
...
main()
{
	...
	echo -e "\e[0;30;43m ***********         Press the NMI button in 15s          *********** \e[0m"
	if [ ${#DeteteCmd} == 0 ] ; then
		echo -e "\e[0;30;43m ***********     NMI 開關只能按一次,重測NMI需要重啟OS     *********** \e[0m"
	else
		echo -e "\e[0;30;43m ***********     NMI 開關即將開始測試,請根據提示操作!     *********** \e[0m"
	fi
	echo -e "\e[0;30;43m ******************************************************************** \e[0m"
	printf "%s\r" "檢查NMI是否短路中，請勿按NMI開關 ..."
	if [ ${#DeteteCmd} == 0 ] ; then
		dmesg | grep -iw "Uhhuh" | grep -iwq "NMI"
		if [ $? == 0 ]; then
			Process 1 "發現NMI開關有短路現象,或你提前按了NMI ..."
			let ErrorFlag++
		fi
	else
		...
		#Before Press
		if [[ $(${DeteteCmd} 2>/dev/null | tr -d " " ) == "${Short:-AA}" ]]; then
			Process 1 "發現NMI開關有短路現象,或你提前按了NMI ..."
			let ErrorFlag++
		fi
	fi
	[ ${ErrorFlag} != 0 ] && exit 1
	printf "%s\n" "可以按${Location}這個NMI開關開始測試了 ..."
	echo
	...
	do	
		...
		do   
			...
			if [ $? == 0 ]; then
				echo
				dmesg -T | grep -iw "Uhhuh" | grep -iw "NMI"
				break 2
			fi
			sleep 0.01
		done
...
declare RealTime=0
declare XmlConfigFile Location ApVersion DeteteCmd Short 
