...
VersionInfo()
{
	...
	local Description="Network external loopback test"
...
`basename $0` [-x lConfig.xml] [-DV]
	...
	return code:
		0 : Exteral loopback pass
		1 : Exteral loopback fail
		2 : File is not exist
		3 : Parameters error
	    Other : Fail
...
GetParametersFrXML  ()
{
	...
	PairArray=($(xmlstarlet sel -t -v "//NetCard/TestCase[ProgramName=\"${BaseName}\"]/Loopback/Pair" -n "${XmlConfigFile}" 2>/dev/null))
...
CreateShellAP ()
{
	local TargetNicIndex=($(echo $1))
	local mod=$(echo $((${#TargetNicIndex[@]}%2)))
	local TargetName=$(echo ${TargetNicIndex[@]} | tr -d ' ')
	local PassFlag="PASSED"
	local FailFlag="fail\|Invalid adapter"

	# Get the usage of celo64e /nic=# /extlb 
	Celo64eExtlbArg=($(${TestTool} /nic=1 /extlb 10 | grep -iE '/extlb \[' | awk -F'-' '{print $1}'|awk -F'[' '{print $2}' | tr -d ']'))

	Rj45Arg=(10 100 1000 10000 10000 400000 7 8 9 10 11 12 13)
	case $mod in
		1)

			cat<<-EOFRJ45>${BaseName}${TargetName}.sh
			#!/bin/bash
			cd ${WorkPath}
			Process()
			{ 	
			...
			FailFlag=0
			rm -rf ${BaseName}${TargetName}.log 2>/dev/null
			
			WaitTime=\$(echo "obase=10;${m}*0.5" | bc )
			WaitTime=\${WaitTime:-1}
			read -t \${WaitTime}
			
			{(${TestTool} /nic=${TargetNicIndex[0]} /extlb ${Rj45Arg[@]:0:${#Celo64eExtlbArg[@]}} /TIMEOUT 60 ) || let FailFlag++ ;} 2>&1 | tee ${BaseName}${TargetName}.log
			PassAmount=\$(cat -v ${BaseName}${TargetName}.log | grep -ic "$PassFlag")
			FailAmount=\$(cat -v ${BaseName}${TargetName}.log | grep -ic "$FailFlag")
			if [ "${TestMode}"x == "serial"x ] && [ ${#pcb} != 0  ] ; then
				cat ${BaseName}${TargetName}.log >> ../PPID/${pcb}.log
			fi
			rm -rf ${BaseName}${TargetName}.log 2>/dev/null

			let TestResult=\${PassAmount}*\${FailAmount}
			if [ \${PassAmount} -ge 1 ] &&  [ \${FailAmount} == 0 ] && [ \$FailFlag == 0 ]; then
				Process 0 "LAN${TargetNicIndex[0]} external loopback test"
			else
				Process 1 "LAN${TargetNicIndex[0]} external loopback test"
				GenerateErrorCode
				exit 1
			fi
			exit 0
		...
		0)
			cat<<-EOF>${BaseName}${TargetName}.sh
			#!/bin/bash
			cd ${WorkPath}
			Process()
			{ 	
			...
			WaitTime=\${WaitTime:-1}
			read -t \${WaitTime}
			{(${TestTool} /sendresp ${TargetNicIndex[@]}  /TIMEOUT 60 ) || let FailFlag++ ;} 2>&1 | tee ${BaseName}${TargetName}.log
			#{(${TestTool} /sendresp ${TargetNicIndex[1]} ${TargetNicIndex[0]}  /TIMEOUT 60 )|| let FailFlag++;} 2>&1 | tee -a ${BaseName}${TargetName}.log
			PassAmount=\$(cat -v ${BaseName}${TargetName}.log | grep -ic "$PassFlag")
			FailAmount=\$(cat -v ${BaseName}${TargetName}.log | grep -ic "$FailFlag")
			#if [ "${TestMode}"x == "serial"x ] && [ ${#pcb} != 0  ] ; then
			#	cat ${BaseName}${TargetName}.log >> ../PPID/${pcb}.log
			...
			if [ \${PassAmount} -ge 1 ] &&  [ \${FailAmount} == 0 ] ; then
				Process 0 "LAN${TargetNicIndex[@]} external loopback test"
			else
				Process 1 "LAN${TargetNicIndex[@]} external loopback test"
				GenerateErrorCode
				exit 1
			fi
			exit 0
		...
		*)
			Process 1 "Bad nic index combination: ${TargetNicIndex[@]}"
			exit 4
		;;
		esac
	chmod 777 ${BaseName}${TargetName}.sh	
}
...
main ()
{
	...
	for ((m=0;m<${#PairArray[@]};m++))
	do
		let M=${m}+1
		LoopNicIndex=($(xmlstarlet sel -t -v "//NetCard/TestCase[ProgramName=\"${BaseName}\"]/Loopback[Pair=\"${PairArray[$m]}\"]/Card/NicIndex" -n "${XmlConfigFile}" 2>/dev/null))
		local LoopNicIndex_1st=$(cat -v .temp/${BaseName}.log | grep -iw "${SoleLanChipset}" | sed -n ${LoopNicIndex[0]}p | grep -iw "${LanChipset[$j]}" | awk '{print $1}') 
		
		if [ ${#LoopNicIndex[@]} == 2 ] ; then
			local LoopNicIndex_2nd=$(cat -v .temp/${BaseName}.log | grep -iw "${SoleLanChipset}" | sed -n ${LoopNicIndex[1]}p | grep -iw "${LanChipset[$j]}" | awk '{print $1}') 
			SubName=$(echo "${LoopNicIndex_1st}${LoopNicIndex_2nd}" | tr -d ' ')
			CreateShellAP "${LoopNicIndex_1st} ${LoopNicIndex_2nd}"
		else
			SubName=$(echo "${LoopNicIndex_1st}" | tr -d ' ')
			CreateShellAP "${LoopNicIndex_1st}"
		fi
		
		if [ ! -s "${BaseName}${SubName}.sh" ] ; then
			Process 1 "No such shell or 0 KB size of file: ${BaseName}${SubName}.sh"
			let ErrorFlag++	
		else
			# For Multi-thread test in X server mode
			chmod 777 ${WorkPath}/${BaseName}${SubName}.sh 2>/dev/null
			echo "${WorkPath}/${BaseName}${SubName}.sh" >> ${MTConfigFile}
			sync;sync;sync
		fi
	done
	...
	if [ "${TestMode}"x == "serial"x ] && [ ${#pcb} != 0 ] ; then
		if [ ${ErrorFlag} != 0 ] ; then
			echoFail "LAN external loopback test"
		else
			echoPass "LAN external loopback test"
		fi >> ${logpath}/${pcb}.log
		sync;sync;sync 
	fi
...
declare XmlConfigFile NicIndexArray TotalAmount LanChipset NicIndex PairArray
declare ErrorFlag=0
...
do
	 case ${argv} in	
		...
		P)
			...
			if [ $? == 0 ] ; then
				printf "%-s\n" "ParallelTest,LanExternalTest"
			else
				printf "%-s\n" "SerialTest,LanExternalTest"
			fi
			exit 1
		;;		
