...
VersionInfo()
{
	...
	local Description="Compare SSID and SVID information"
...
`basename $0` [-x lConfig.xml] [-DV]
	...
	return code:
		0 : Compare SSID-SVID pass
		1 : Compare SSID-SVID fail
		2 : File is not exist
		3 : Parameters error
	    Other : Fail
	...
	<NetCard>		
		<TestCase>			
			...
			<!--範例說明
				...
				<SSIDSVID>8086-1F41,忽略請填寫NULL</SSIDSVID>
			-->
						
			<Card>
				<NicIndex>1</NicIndex>
				<Chipset>I354</Chipset>
				<SSIDSVID>8086-1F41</SSIDSVID>
			</Card>
...
GetParametersFrXML  ()
{
	...
	SSIDSVID=($(xmlstarlet sel -t -v "//NetCard/TestCase[ProgramName=\"${BaseName}\"]/Card/SSIDSVID" -n "${XmlConfigFile}" 2>/dev/null))
...
ShowMTProcess ()
{
	...
	ShowTitle "Multithreading tool for LAN SSID-SVID compared test"
...
MergeLog()
{
	...
	echo "Multithreading test tool for LAN SSID-SVID compared test, all test logs" >> ./logs/${BaseName}.log
...
CreatedCompareSSIDSVID()
{
	for ((m=0;m<${#NicIndexArray[@]};m++))
	do
		let M=$m+1
	cat<<-SSIDSSVD > ${BaseName}${NicIndexArray[$m]}.sh
		echoPass()
		 { 	local String=\$@ 
			echo -en "\e[1;32m \$String\e[0m"
			str_len=$(echo \${#String}) 
		...
		GetSSIDSSVD ()
		{
		#${NicIndexArray[$m]} ${SSIDSVID[$m]} 
		local TargetNicIndex=\$1
		local TargetSSIDSVID=\$2
		
		echo "\${TargetSSIDSVID}" | grep -iwq "NULL"
		if [ \$? == 0 ] ; then
			exit 0
		fi

		# Dump the SSID-SVID
		# If the current SVID is equal to the target SVID address,check pass
		CurSSIDSVID[\$TargetNicIndex]=\$(${ProgramTool} /nic=\${TargetNicIndex} /pciinfo |  grep -Ev '^$' | tail -n1 | awk '{print \$6}')
		if [ "\${CurSSIDSVID[\$TargetNicIndex]}x" == "\${TargetSSIDSVID}"x ] ; then
			echoPass "Compare LAN\${TargetNicIndex} SSID-SVID(\$TargetSSIDSVID)"
			echo ${SSIDSVID[$m]} > ../PPID/SSIDSVID${M}.TXT 2>/dev/null
			sync;sync;sync
			
			# Confirm the SSID-SSVD.TXT is valid
			MacAddrTemp=\$(cat -v ../PPID/SSIDSVID${M}.TXT 2>/dev/null | head -n1)
			if [ -z "\$MacAddrTemp" ] ; then
				echoFail "../PPID/SSIDSVID${M}.TXT abnormal or empty"
				GenerateErrorCode
				exit 1
			fi
			...
			echoFail "Compare LAN\${TargetNicIndex} SSID-SVID"
			echo " Current SSID-SVID: \${TargetSSIDSVID}"
			echo "Standard SSID-SVID: \${CurSSIDSVID[\$TargetNicIndex]}"
			GenerateErrorCode
			exit 1
		fi
		}
	SSIDSSVD
		...
		echo "GetSSIDSSVD  ${NicIndexArray[$m]} ${SSIDSVID[$m]}" >> ${BaseName}${NicIndexArray[$m]}.sh
		sync;sync;sync	
...
main ()
{
	...
	if [ "${TestMode}"x == "serial"x ] && [ ${#pcb} != 0 ] ; then
		if [ ${ErrorFlag} != 0 ] ; then
			echoFail "Check the SSID and SVID of net card"
		else
			echoPass "Check the SSID and SVID of net card"
		fi >> ${logpath}/${pcb}.log
		sync;sync;sync 
	fi		
	if [ ${ErrorFlag} != 0 ] ; then
...
declare XmlConfigFile NicIndexArray TotalAmount SSIDSVID LanChipset NicIndex
declare ErrorFlag=0
...
do
	 case ${argv} in	
		...
		P)
			...
			if [ $? == 0 ] ; then
				printf "%-s\n" "ParallelTest,CheckSSIDSVID"
			else
				printf "%-s\n" "SerialTest,CheckSSIDSVID"
			fi
			exit 1
		;;		
