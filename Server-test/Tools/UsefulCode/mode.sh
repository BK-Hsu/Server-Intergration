...
`basename $0` [-x lConfig.xml] [-DV]
	...
	return code:
	   0 : Check BIOS pass,BIOS version and release date are match the standard
	   1 : Check BIOS fail
	   2 : File is not exist
	   3 : Parameters error
	other: Check fail
...
DumpXML()
{
	...
	<BIOS>
		<TestCase>
			...
			<ErrorCode>TXS3E|WiFi test fail</ErrorCode>
			
			<!-- ChkBios.sh: for Check BIOS版本和發行日期 --> 
			<Version>ES115ISC.151</Version>
			<ReleaseDate>06/03/2016</ReleaseDate>
			<!-- 置空DST則不再測試（卡），但會顯示在屏幕上 --> 
			<DST>2</DST>
