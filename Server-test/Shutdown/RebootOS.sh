...
main ()
{
	...
	ShowMsg --b "Rebbot application version: $ProgramVer "
	ShowMsg --2 "-----------------------------------------------------"
	ShowMsg --3 "[1] Do not turn Off AC-Power. "
	ShowMsg --e "[2] Do not press reset button."
	...
	do
		...
		for ((p=5;p>=0;p--))
		do   
			echo -ne "\e[1;33m`printf "\rOS will reboot after %02d sec,press [Y] execute at once ...\n" "${p}"`\e[0m"
			read -t1 -n1 -s Ans
